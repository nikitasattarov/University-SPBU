...
lines <- readLines("saferoads.jsonl")[1:50000]

data <- mapply(parse_accident, lines, 1:length(lines), SIMPLIFY = FALSE)[1:50]
...
plot_russia() +
    geom_point(data = subset(accidents, lat > 10 & fatalities / participants_count > 0.75),
        aes(x = long, y = lat),
        color = "red", alpha = 0.5, size = 1) +
...
ggplot(data = subset(accidents, type == "Столкновение" | type == "Съезд с дороги") %>%
    group_by(type) %>%
